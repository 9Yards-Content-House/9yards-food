    ...
    <nav className="fixed bottom-0 left-0 right-0 z-50 bg-card/95 backdrop-blur-md border-t border-border lg:hidden safe-area-bottom">
      <div className="flex items-center justify-around h-18 px-2">
        {navItems.map((item) => {
          ...
          return (
            <Link
              key={item.href}
              to={item.href}
              className={`flex flex-col items-center justify-center gap-1 min-w-[56px] min-h-[56px] py-2 px-3 rounded-xl transition-colors relative ${
        ...
        <button
          ...
          className="flex flex-col items-center justify-center gap-1 min-w-[56px] min-h-[56px] py-2 px-3 rounded-xl text-muted-foreground hover:text-green-600 transition-colors"
